...
describe("interactive UI", () => {
  ...
  it("shows and resets the schedule empty state", async () => {
    ...
    await user.selectOptions(screen.getByLabelText("Trainer"), "coach-two");
    expect(screen.getByText("No demonstration sessions match.")).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "Reset filters" }));
    expect(
      screen.queryByText("No demonstration sessions match."),
    ).not.toBeInTheDocument();
  });
  it("changes membership billing selection", async () => {
    const user = userEvent.setup();
    render(<PricingSelector plans={memberships} />);
  ...
  it("announces trial validation errors", async () => {
    const user = userEvent.setup();
  ...
  it("submits contact form in mock mode", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: true }));
    const user = userEvent.setup();
    render(<ContactForm />);
    await user.type(screen.getByLabelText("Full name"), "Maya Haddad");
    await user.type(
      screen.getByLabelText("Phone or email"),
      "maya@example.com",
    );
    await user.type(
      screen.getByLabelText("Message"),
      "I would like class information.",
    );
    await user.click(
      screen.getByRole("checkbox", { name: /demonstration inquiry/ }),
    );
  ...
  it("stores rejected optional cookies", async () => {
    const user = userEvent.setup();
    render(<CookieConsent />);
    expect(
      await screen.findByRole("dialog", { name: "Cookie choices" }),
    ).toBeInTheDocument();
    await user.click(
      screen.getByRole("button", { name: "Reject non-essential" }),
    );
    expect(localStorage.getItem("forge-cookie-preferences")).toContain(
      '"analytics":false',
    );
  });
});
